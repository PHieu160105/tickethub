...
#   timezone.utc: hằng số múi giờ timezone.utc+0
...
def combine_show_datetime(show_date: date, show_time: time) -> datetime:
    """Ghép ngày diễn và giờ diễn thành `datetime` có múi giờ timezone.utc.
    ...
    Đầu ra:
        datetime - đã ghép ngày + giờ + múi giờ timezone.utc
    """

    # datetime.combine(): Python có sẵn method
    #   Ghép date và time thành datetime
    #   tzinfo=timezone.utc: gắn múi giờ timezone.utc
    return datetime.combine(show_date, show_time, tzinfo=timezone.utc)


def _as_utc(value: datetime | None) -> datetime | None:
    """Chuẩn hóa datetime naive từ DB thành datetime có múi giờ timezone.utc.
    
    MỤC ĐÍCH: Database có thể trả về datetime không có múi giờ.
    Hàm này gắn timezone.utc vào để so sánh an toàn với các datetime khác.
    
    Đầu vào:
        value: datetime | None - thời gian từ database
        
    Đầu ra:
        datetime | None - đã có múi giờ timezone.utc, hoặc None nếu đầu vào None
    """
    ...
    # value.replace(tzinfo=timezone.utc): tạo bản sao với múi giờ mới
    # timezone.utc: hằng số từ datetime module
    return value if value.tzinfo else value.replace(tzinfo=timezone.utc)


def _event_range_to_datetimes(start_date: date, end_date: date) -> tuple[datetime, datetime]:
    """Tạo khoảng thời gian timezone.utc đại diện cho ngày bắt đầu/kết thúc của sự kiện.
    ...
    - start_date → 00:00:00 timezone.utc của ngày đó
    - end_date → 23:59:59 timezone.utc của ngày đó
...
def _build_ticket_from_seat(show_id: int, seat: Seat) -> Ticket:
    return Ticket(
        show_id=show_id,
        ticket_tier_id=seat.zone_id,
        seat_id=seat.id,
...
async def sync_show_ticket_inventory(session: AsyncSession, show: Show) -> None:
    """Đồng bộ ticket inventory từ seat legacy của show."""
    ...
    live_seat_ids = {seat.id for seat in seats}
    ticket_by_seat_id = {ticket.seat_id: ticket for ticket in tickets if ticket.seat_id is not None}

    for seat in seats:
        ticket = ticket_by_seat_id.get(seat.id)
        if ticket is None:
            session.add(_build_ticket_from_seat(show.id, seat))
            continue

        ticket.show_id = show.id
        ticket.ticket_tier_id = seat.zone_id
        ticket.seat_id = seat.id
        ticket.locked_by_user_id = seat.locked_by_user_id
        ticket.is_admin_locked = seat.is_admin_locked
        ticket.lock_expires_at = seat.lock_expires_at
        if ticket.order_id is None and ticket.status != SeatStatus.SOLD:
            ticket.seat_label = seat.seat_label
            ticket.row_label = seat.row_label
            ticket.seat_number = seat.seat_number
            ticket.x_coord = seat.x_coord
            ticket.y_coord = seat.y_coord
            ticket.price = seat.price
            ticket.status = seat.status

    for ticket in tickets:
        if ticket.seat_id is None or ticket.seat_id in live_seat_ids:
            continue
...
async def _clone_layout_inventory(
    ...
    """Clone seat mẫu của venue layout thành tồn kho bán vé cho show.
    ...
    # ============================================================
    # BƯỚC 2: TẠO TICKET TIER MẶC ĐỊNH CHO LAYOUT
    # ============================================================
    if not template_seats:
        return

    section_ids = {seat.section_id for seat in template_seats if seat.section_id is not None}
    sections = list(await session.scalars(select(Section).where(Section.id.in_(section_ids)))) if section_ids else []
    ...
    fallback_zone = SeatZone(
        ...
        code="GEN",
        name="Khu vực chung",
        row_count=max(len({seat.row_label for seat in template_seats if seat.row_label}), 1),
        seats_per_row=max(max((seat.seat_number for seat in template_seats), default=0), 1),
        price=Decimal("0"),
        color="#024ddf",
    )
    ...
    if primary_section is not None:
        fallback_zone.code = primary_section.code
        fallback_zone.name = primary_section.name
        fallback_zone.price = primary_section.price_base
        fallback_zone.color = primary_section.color
    ...
    for template_seat in template_seats:
        section = section_by_id.get(template_seat.section_id or -1)
        cloned_seats.append(
            Seat(  # ORM model tự viết
                ...
                price=section.price_base if section else template_seat.price,
...
async def create_show_with_inventory(
    ...
    show = Show(
        ...
        venue=payload.location or (venue.name if venue else ""),
        start_at=start_at,                               # Datetime timezone.utc
        end_at=end_at,                                   # Datetime timezone.utc
        status=payload.status,                           # Enum EventStatus
        seat_source=payload.seat_source,
...
async def list_event_max_prices_for_event_ids(
    ...
    stmt = (
        select(
            Show.event_id,
            func.max(Ticket.price).label("max_price"),
        )
    ...
    if live_only:
        stmt = stmt.where(Show.status == EventStatus.LIVE, Show.is_deleted.is_(False))
...
async def update_show_zone(
    ...
    blocked = await session.scalar(
        select(func.count(Ticket.id)).where(
            ...
            Ticket.status.in_([SeatStatus.SOLD, SeatStatus.LOCKED]),
        )
    )
    if blocked:
        raise HTTPException(
        ...
        max_price = float(
            (
                await session.scalar(
                    select(func.max(Ticket.price))
                    .join(Show, Ticket.show_id == Show.id)
                    .where(Show.event_id == event.id)
                )
            )
            or 0
        )
...
async def get_show_seat_matrix(
    ...
    tickets = list(
        await session.scalars(
            ...
            .order_by(Ticket.ticket_tier_id, Ticket.row_label, Ticket.seat_number, Ticket.id)
        )
    )

    now = datetime.now(timezone.utc)  # Python có sẵn: thời gian hiện tại
    ...
    seat_row_index_map: dict[int, int] = {}
    seat_ids = [ticket.seat_id for ticket in tickets if ticket.seat_id is not None]
    if seat_ids:
        seat_rows = (
            await session.execute(
                select(Seat.id, Seat.row_index).where(Seat.id.in_(seat_ids))
            )
        ).all()
        seat_row_index_map = {row.id: row.row_index for row in seat_rows}
    ...
    if include_user_details and tickets:
        ...
        # Dict comprehension: map ticket_id → PurchaseInfo
        sold_user_map = {
            row.ticket_id: SeatPurchaseInfoResponse(
    ...
    for ticket in tickets:
        ...
        normalized_status = (
            SeatStatus.LOCKED
            if ticket.is_admin_locked and ticket.status != SeatStatus.SOLD
            else ticket.status
        )
        
        # Chuẩn hóa lock_expires_at
        lock_expires = _as_utc(ticket.lock_expires_at)  # Hàm tự viết
        ...
        if (
            ...
            and ticket.locked_by_user_id is not None
        ):
            # .get(): Python dict method - lấy value theo key
            locked_by_user = locked_user_map.get(ticket.locked_by_user_id)

        # Lấy thông tin người mua (nếu có và admin mode)
        sold_to_user = (
            sold_user_map.get(ticket.id)
            if include_user_details and ticket.status == SeatStatus.SOLD
            else None
        )
        ...
        seat_responses.append(
            SeatResponse(  # lược đồ Pydantic tự viết
                ...
                row_index=seat_row_index_map.get(ticket.seat_id, 0) if ticket.seat_id is not None else 0,
                row_label=ticket.row_label or "",
                seat_number=ticket.seat_number or 0,
                seat_label=ticket.seat_label or "",
                price=Decimal(str(ticket.price)),  # Decimal Python có sẵn
                status=normalized_status,
                lock_expires_at=lock_expires,
                # Chỉ True nếu user đang đăng nhập VÀ là người giữ ghế này
                is_locked_by_me=(
                    current_user_id is not None
                    and ticket.locked_by_user_id == current_user_id
                ),
